import { getPoolContext, type PoolContext } from "../core/poolContext";
...
const SOL_MINTS = new Set<string>([
...
const MID_LINK_ANCHOR_CLASS =
  "inline-flex items-center gap-1.5 pl-2 pr-3 py-1.5 rounded-lg border border-v2-border-secondary " +
  "bg-transparent hover:bg-v2-base-1 text-v2-text-primary transition-colors w-fit";
...
let LAST_POSITION_MODE: "fixed" | "anchored" | null = null;
let isPositionLocked = false;
let pendingAnchorRect: { left: number; top: number; width: number; height: number } | null = null;
...
let liveExternalLinksConfig: ExternalLinksConfig | null = null;
let midLinksLayoutRaf: number | null = null;
let midLinksGlobalEventsBound = false;
let midLinksLastHiddenCount = 0;
let midLinksResizeObserver: ResizeObserver | null = null;
let midLinksWatchdogTimer: number | null = null;
let samePairPollTimer: number | null = null;
let samePairPollInFlight = false;
let samePairAbortController: AbortController | null = null;
let samePairConsecutiveErrors = 0;
let samePairLastRenderedSignature = "";
let samePairLastRouteKey = "";
let samePairLastUpdatedAt = 0;
let samePairSortState: { key: SamePairSortKey; direction: SamePairSortDirection } = {
  key: "volume5m",
  direction: "desc"
};
const cachedExternalLinksByLabel = new Map<string, string>();
...
type SamePairPoolItem = {
  poolAddress: string;
  baseMint: string | null;
  quoteMint: string | null;
  metrics: SamePairMetrics;
};

type SamePairFetchResult = {
  items: SamePairPoolItem[];
  source: "data-api" | "dlmm-api";
  usedVolumeProxy: boolean;
};
type SamePairSortKey = "binStep" | "baseFee" | "tvl" | "volume5m" | "volume1h" | "volume24h" | "apr";
type SamePairSortDirection = "asc" | "desc";
...
function normalizeExternalLinksConfig(raw: unknown): ExternalLinksConfig {
  ...
  if (cfg.links && typeof cfg.links === "object") {
    for (const item of EXTERNAL_LINK_ITEMS) {
      ...
      if (typeof value === "boolean") normalized.links[item.key] = value;
    }
  }
  let selected = 0;
  for (const item of EXTERNAL_LINK_ITEMS) {
...
function readExternalLinksConfigFromStorage(): Promise<ExternalLinksConfig> {
  if (!chrome.storage?.local) return Promise.resolve(DEFAULT_EXTERNAL_LINKS_CONFIG);
  return new Promise((resolve) => {
    chrome.storage.local.get([EXTERNAL_LINKS_SETTINGS_KEY], (result) => {
      if (chrome.runtime.lastError) {
        resolve(DEFAULT_EXTERNAL_LINKS_CONFIG);
        return;
      }
      resolve(normalizeExternalLinksConfig(result?.[EXTERNAL_LINKS_SETTINGS_KEY]));
    });
  });
}

function getExternalLinksConfig(): Promise<ExternalLinksConfig> {
  if (liveExternalLinksConfig) return Promise.resolve(liveExternalLinksConfig);
...
function findAllInAllRoots(selector: string): Element[] {
  const found: Element[] = [];
...
function queryButtonsInAllRoots(): HTMLButtonElement[] {
...
function pickBestVisibleElement(candidates: Element[]): HTMLElement | null {
...
function findJupiterPrimaryButton(): HTMLElement | null {
  ...
  const logoCandidates = findAllInAllRoots("img[alt='Jupiter aggregator'], img[src*='jup.ag/svg/jupiter-logo.svg'], img[src*='jupiter-logo.svg']")
    ...
    .filter((el): el is HTMLElement => el instanceof HTMLElement && el.id !== BTN_ID);
...
function toSimpleRect(rect: DOMRect): { left: number; top: number; width: number; height: number } {
  return {
...
function isRectStableEnough(
  previous: { left: number; top: number; width: number; height: number },
...
function applyFloatingButtonPosition(btn: HTMLButtonElement): void {
  ...
  if (jupRect && document.readyState === "complete") {
    ...
    if (LAST_POSITION_MODE !== "anchored") {
      console.debug(LOG_PREFIX, "Button anchored to Jupiter", {
        left: btn.style.left,
        top: btn.style.top,
        size: anchoredSize
      });
...
function findWithdrawButton(): HTMLButtonElement | null {
  const buttons = queryButtonsInAllRoots();
  for (const btn of buttons) {
    if (!(btn instanceof HTMLButtonElement)) continue;
    const text = (btn.textContent || "").replace(/\s+/g, " ").trim().toLowerCase();
...
function findCreatePositionButton(): HTMLButtonElement | null {
  const buttons = queryButtonsInAllRoots();
  for (const btn of buttons) {
    if (!(btn instanceof HTMLButtonElement)) continue;
    const text = (btn.textContent || "").replace(/\s+/g, " ").trim().toLowerCase();
...
function findNewPositionCreateButton(): HTMLButtonElement | null {
...
function findClosedPositionsButton(): HTMLButtonElement | null {
  const buttons = queryButtonsInAllRoots();
  for (const btn of buttons) {
    if (!(btn instanceof HTMLButtonElement)) continue;
    const text = (btn.textContent || "").replace(/\s+/g, " ").trim().toLowerCase();
...
function findUiTemplateButton(): HTMLButtonElement | null {
...
function hasCloseAllCompletionSignal(): boolean {
...
function getTransactionNotificationTextAndSignature(): { text: string; signature: string } | null {
  const panel = document.querySelector("[data-sentry-component='TransactionNotification']");
...
function hasInstructionError6039(text: string): boolean {
  return (
...
function consumeExitSell6039FailureSignal(): boolean {
...
async function waitForCloseAllCompletionSignal(): Promise<boolean> {
  const startedAt = Date.now();
...
function startExitSellRelay(): void {
  ...
  const tick = (): void => {
    ...
    if (consumeExitSell6039FailureSignal()) {
      ...
      console.debug(LOG_PREFIX, `Exit sell 6039 failure ${exitSell6039FailureCount}/${EXIT_SELL_MAX_6039_FAILURES}`);
      if (exitSell6039FailureCount >= EXIT_SELL_MAX_6039_FAILURES) {
        console.debug(LOG_PREFIX, "Exit sell relay stopped after 3 InstructionError Custom 6039 failures");
        stopExitSellRelay();
        return;
      }
    }
...
async function runWithdrawAndAxiomFlow(triggerBtn: HTMLButtonElement, withdrawBtn: HTMLButtonElement | null): Promise<void> {
  ...
  try {
    ...
    if (withdrawBtn) {
      ...
      console.debug(LOG_PREFIX, "Triggered Close All");
    ...
    if (withdrawBtn) {
      ...
      if (!hasSignal) {
        console.debug(LOG_PREFIX, "Close All completion signal wait timed out, starting sell relay anyway");
      }
    }
    startExitSellRelay();
...
function applyExitButtonFallbackStyle(btn: HTMLButtonElement): void {
  btn.className = "";
...
function applySellButtonInlineStyle(btn: HTMLButtonElement, template: HTMLButtonElement): void {
...
function ensureSellButtonContent(btn: HTMLButtonElement): void {
...
function ensureWithdrawAxiomButton(): void {
  ...
  let host =
    createPositionBtn?.parentElement ||
    withdrawBtn?.parentElement ||
    closedPositionsBtn?.parentElement ||
    (resolveAnchor() as HTMLElement | null);
  if (host?.id === ACTIONS_WRAP_ID && host.parentElement instanceof HTMLElement) {
  ...
  if (closedPositionsBtn?.parentElement) {
...
function findTabsListHost(): HTMLElement | null {
  const candidate =
  ...
  if (createBtn?.parentElement instanceof HTMLElement) return createBtn.parentElement;
  const closeAllBtn = findWithdrawCloseAllButton() || findWithdrawButton();
  if (closeAllBtn?.parentElement instanceof HTMLElement) return closeAllBtn.parentElement;
  const fallback = resolveAnchor();
  return fallback instanceof HTMLElement ? fallback : null;
}

function findExternalLinksSourceHost(): HTMLElement | null {
...
function resolveExternalLinkUrl(
  key: string,
  poolAddress: string,
  tokenMint: string
): string | null {
  switch (key) {
    ...
    case "birdeye":
      return tokenMint && poolAddress
        ? `https://birdeye.so/token/${tokenMint}/${poolAddress}?chain=solana`
        : null;
    case "geckoTerminal":
...
function readLinkLabel(anchor: HTMLAnchorElement): string {
  const span = anchor.querySelector("span");
  const raw = (span?.textContent || anchor.textContent || "").trim();
  return raw.replace(/\s+/g, " ").trim();
}

function inferExternalLinkKeyFromHref(href: string): string | null {
...
function createFallbackExternalIcon(): SVGSVGElement {
...
function closeMidLinksMenu(): void {
...
function openMidLinksMenu(anchorBtn: HTMLButtonElement): void {
  const menu = getOrCreateMidLinksPortalMenu();
  if (!(menu instanceof HTMLDivElement)) return;
  menu.textContent = "";
  const hiddenLinks = Array.from(document.querySelectorAll(`#${MID_LINKS_ROW_ID} a`)).filter(
    (el): el is HTMLAnchorElement => el instanceof HTMLAnchorElement && getComputedStyle(el).display === "none"
  );
...
function applyMidLinksOverflowLayout(): void {
  ...
  if (
    actionsWrap instanceof HTMLElement &&
    actionsWrap.parentElement === wrap.parentElement
  ) {
  ...
  const computeVisibleWidth = (): number => {
...
function bindMidLinksGlobalEvents(): void {
...
function bindMidLinksResizeObserver(): void {
...
function samePairIsRecord(value: unknown): value is Record<string, unknown> {
  return !!value && typeof value === "object" && !Array.isArray(value);
}

function samePairToNumber(value: unknown): number | null {
...
function samePairReadString(obj: Record<string, unknown>, keys: string[]): string | null {
...
function samePairReadNumber(obj: Record<string, unknown>, keys: string[]): number | null {
  for (const key of keys) {
...
function samePairNormalizeMetricKey(key: string): string {
  return key.toLowerCase().replace(/[^a-z0-9]/g, "");
}

function samePairReadWindowValue(value: unknown, keys: string[]): number | null {
...
function samePairBuildMintKey(baseMint: string, quoteMint: string): string {
  return [baseMint, quoteMint].sort().join("-");
}

function samePairMintsMatch(item: SamePairPoolItem, baseMint: string, quoteMint: string): boolean {
...
function samePairLikelyAddress(value: string | null): boolean {
  return !!value && /^[1-9A-HJ-NP-Za-km-z]{32,44}$/.test(value);
}

function samePairNormalizePoolCandidate(
  candidate: Record<string, unknown>,
  options: { useMin30Proxy: boolean }
): SamePairPoolItem | null {
  const nested =
...
function samePairExtractCandidates(payload: unknown): Record<string, unknown>[] {
...
function samePairSortAndTrim(items: SamePairPoolItem[], maxRows: number): SamePairPoolItem[] {
  const dedup = new Map<string, SamePairPoolItem>();
...
function samePairCollectItemsFromPayload(
  payload: unknown,
  baseMint: string,
  quoteMint: string,
  options: { useMin30Proxy: boolean }
  ...
  const normalized: SamePairPoolItem[] = [];
...
async function samePairFetchJson(url: string, signal: AbortSignal): Promise<unknown> {
  const res = await fetch(url, { credentials: "omit", signal });
  if (!res.ok) throw new Error(`HTTP ${res.status} for ${url}`);
  return res.json();
}

function samePairIsHttpNotFoundError(error: unknown): boolean {
  const message = (error as { message?: string })?.message || "";
  return /HTTP 404/i.test(message);
}

function samePairBuildLexicalKeys(baseMint: string, quoteMint: string): string[] {
  const [a, b] = [baseMint, quoteMint].sort();
  return [`${a}-${b}`, `${a}_${b}`, `${a}:${b}`];
}

function samePairBuildDataApiUrls(baseMint: string, quoteMint: string): string[] {
...
function samePairBuildDlmmApiUrls(baseMint: string, quoteMint: string): string[] {
  const urls = new Set<string>();
  const lexicalKeys = samePairBuildLexicalKeys(baseMint, quoteMint);
  for (const lexical of lexicalKeys) {
...
async function samePairFetchFromDataApi(
  baseMint: string,
  quoteMint: string,
  signal: AbortSignal
): Promise<SamePairFetchResult | null> {
...
async function samePairFetchFromDlmmApi(
  baseMint: string,
  quoteMint: string,
  signal: AbortSignal
): Promise<SamePairFetchResult | null> {
...
async function samePairFetchPools(context: PoolContext, signal: AbortSignal): Promise<SamePairFetchResult> {
...
function samePairFindAnchorForm(): HTMLFormElement | null {
...
function ensureSamePairWidgetRoot(): HTMLDivElement | null {
...
function samePairRemoveWidget(): void {
  const widget = document.getElementById(SAME_PAIR_WIDGET_ID);
  if (widget) widget.remove();
}

function samePairSetHeader(text: string, status: string): void {
...
function samePairFormatFeePercent(value: number | null): string {
...
function samePairStatusText(): string {
  const seconds = Math.max(0, Math.floor((Date.now() - samePairLastUpdatedAt) / 1000));
  return `${seconds}s ago`;
}

function samePairSortValue(item: SamePairPoolItem, key: SamePairSortKey): number | null {
...
function samePairSortItems(items: SamePairPoolItem[]): SamePairPoolItem[] {
...
function samePairSortIndicator(key: SamePairSortKey): string {
  if (samePairSortState.key !== key) return "";
  return samePairSortState.direction === "asc" ? " ▲" : " ▼";
}

function samePairToggleSort(key: SamePairSortKey): void {
...
function samePairDisplayPair(item: SamePairPoolItem, context: PoolContext): string {
...
function samePairRefreshHeaderAge(): void {
...
function samePairFormatUsd(value: number | null): string {
  if (value === null || !Number.isFinite(value)) return "--";
...
function samePairFormatPrice(value: number | null): string {
  if (value === null || !Number.isFinite(value)) return "--";
  const abs = Math.abs(value);
...
function samePairFormatPercent(value: number | null): string {
  if (value === null || !Number.isFinite(value)) return "--";
  const normalized = Math.abs(value) <= 1 ? value * 100 : value;
  return `${normalized.toFixed(2)}%`;
}

function samePairRenderBody(children: Node[]): void {
  const root = ensureSamePairWidgetRoot();
  if (!root) return;
...
function samePairRenderLoading(): void {
...
function samePairRenderError(errorText: string, nextDelayMs: number): void {
...
function samePairBuildTargetUrl(poolAddress: string): string {
  const next = new URL(window.location.href);
  next.pathname = `/dlmm/${poolAddress}`;
  return next.toString();
}

function samePairRenderList(result: SamePairFetchResult, context: PoolContext): void {
  ...
  const columns: Array<{ label: string; sortKey: SamePairSortKey | null }> = hasDynamicFee
    ? [
  ...
  for (const col of columns) {
    ...
    if (col.sortKey) {
      ...
      th.addEventListener("click", () => {
        samePairToggleSort(col.sortKey as SamePairSortKey);
...
function samePairClearPollTimer(): void {
...
function samePairResetPollingState(): void {
...
function samePairStopPolling(removeWidget = false): void {
...
function samePairComputeBackoffMs(): number {
...
function samePairSchedulePoll(delayMs: number): void {
  samePairClearPollTimer();
...
async function samePairRunPoll(): Promise<void> {
...
async function ensureMiddleExternalLinks(): Promise<void> {
  ...
  const prepared: Array<{ label: string; href: string }> = [];
  const sourceAnchorsByKey = new Map<string, HTMLAnchorElement>();
  if (source) {
  ...
  for (const item of prepared) {
    ...
    if (sourceAnchor) {
      const iconEl =
        sourceAnchor.querySelector("img") ||
        sourceAnchor.querySelector("div[data-sentry-component]") ||
        sourceAnchor.querySelector("svg");
  ...
  moreBtn.textContent = "+0 \u25BE";
  moreBtn.className =
    "inline-flex items-center justify-between gap-1.5 rounded-md border border-v2-border-secondary " +
    "bg-transparent hover:bg-v2-base-1 text-v2-text-primary transition-colors text-xsm py-1 px-2";
...
function ensureNewPositionAxiomButton(): void {
...
async function syncExternalLinksSettingsIfChanged(): Promise<void> {
...
function init(): void {
  ...
  if (chrome.storage?.onChanged) {
    chrome.storage.onChanged.addListener((changes, areaName) => {
      ...
      const entry = changes?.[EXTERNAL_LINKS_SETTINGS_KEY];
      if (!entry) return;
